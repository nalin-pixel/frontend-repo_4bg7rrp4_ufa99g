...
import { ShieldCheck, Cog, Megaphone, Users, CreditCard, BarChart3, Zap, Sparkles } from 'lucide-react'
  ...
  return (
    <section id="features" className="relative py-24">
      ...
      >
        ...
        <motion.div className="absolute -top-24 right-10 h-40 w-40 rounded-full bg-fuchsia-500/20 blur-2xl" animate={{ y: [0, 20, 0] }} transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }} />
        <motion.div className="absolute bottom-0 left-10 h-28 w-28 rounded-full bg-blue-500/20 blur-2xl" animate={{ y: [0, -15, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut', delay: 0.4 }} />
      </motion.div>
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map(({ icon: Icon, title, desc }, idx) => (
            <motion.div
              ...
              className="group rounded-2xl border border-white/10 bg-slate-900/40 p-5 text-white shadow-[0_10px_30px_rgba(0,0,0,0.25)] overflow-hidden"
            >
              <motion.div className="absolute -right-10 -top-10 h-28 w-28 rounded-full bg-white/5 blur-xl" whileHover={{ scale: 1.2 }} />
        ...
        <div className="mt-14 overflow-hidden">
          ...
          >
            {[Zap, Sparkles, ShieldCheck, BarChart3, Users, Megaphone].map((Icon, i) => (
              <div key={i} className="flex items-center gap-2 whitespace-nowrap text-white/50">
                <Icon className="h-5 w-5" />
                <span className="text-sm">Built for scale</span>
              </div>
            ))}
            {[Zap, Sparkles, ShieldCheck, BarChart3, Users, Megaphone].map((Icon, i) => (
              <div key={`dup-${i}`} className="flex items-center gap-2 whitespace-nowrap text-white/50">
                <Icon className="h-5 w-5" />
                <span className="text-sm">Built for scale</span>
              </div>
            ))}
